...
def snapshot(model, optimizer, config, step, gpus=[0], tag=None, scheduler=None, graph_model=None):
    ...
    if graph_model is not None:
        ...
        file_name = graph_model + "_snapshot_{}.pth"
        file_names = graph_model + "_snapshot_{:07d}.pth"
        torch.save(model_snapshot,
                   os.path.join(config.save_dir, file_name.format(tag)
                   if tag is not None else
                   file_names.format(step)))
        ...
        config_save.test.test_model_name = file_name.format(
            tag) if tag is not None else file_names.format(step)
