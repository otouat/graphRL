...
class Agent():
    def __init__(self, alpha=0.003, beta=0.003, input_dims=[8], env=None, gamma=0.99, n_actions=2, max_size=1000000, tau=0.005, layer1_size=256, layer2_size=256, batch_size=256, reward_scale=2):
        self.gamma = gamma
        self.tau = tau
        ...
        self.update_network = reward_scale
    ...
    def update_network_parameters(self, tau=None):
        ...
        value_params_state_dict = dict(value_params)

        for name in value_params_state_dict:
            value_params_state_dict[name] = tau * value_params_state_dict[name].clone() + (1 - tau) * \
